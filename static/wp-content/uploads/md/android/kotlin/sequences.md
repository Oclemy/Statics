...
* _Stateless_ operations require no state and process each element independently, for example, `map()` or `filter()`.
...
Otherwise, the operation is _terminal_. Examples of terminal operations are `toList()`
