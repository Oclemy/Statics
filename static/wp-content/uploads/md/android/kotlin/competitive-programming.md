...
See Get started with Kotlin/JVM on how to set up development
...
while loop and mutable variables that are denoted in Kotlin with 
var:
...
Note the use of `private` visibility modifier here.
...
idioms like "take the value if it is positive or else use length" with a concise and readable 
...
string templates. However, care must be taken when output 
...
reference section of the website starting from basic syntax. 
