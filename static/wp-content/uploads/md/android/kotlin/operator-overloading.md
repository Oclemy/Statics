...
predefined symbolic representation (like `+` or `*`) and precedence. To implement an operator, provide a member function
or an extension function with a specific name for the corresponding type. This type becomes the left-hand side type
...
> These operations, as well as all the others, are optimized for basic types and do not introduce 
