...
The next step is to [parse](https://en.wikipedia.org/wiki/Parsing#Computer_languages) each string into an object called an expression, represented by the Julia type [`Expr`](https://docs.julialang.org/../../base/base/#Core.Expr):
...
* a [`Symbol`](https://docs.julialang.org/../../base/base/#Core.Symbol) identifying the kind of expression. A symbol is an [interned string](https://en.wikipedia.org/wiki/String_interning) identifier (more discussion below).
...
The [`dump`](https://docs.julialang.org/../../base/io-network/#Base.dump) function provides indented and annotated display of `Expr` objects:
...
(:call, :/, (:call, :+, 4, 4), 2)
```
The `:` character has two syntactic purposes in Julia. The first form creates a [`Symbol`](https://docs.julialang.org/../../base/base/#Core.Symbol), an [interned string](https://en.wikipedia.org/wiki/String_interning) used as one building-block of expressions:
...
Symbol
```
The [`Symbol`](https://docs.julialang.org/../../base/base/#Core.Symbol) constructor takes any number of arguments and creates a new symbol by concatenating their string representations together:
...
In the context of an expression, symbols are used to indicate access to variables; when an expression is evaluated, a symbol is replaced with the value bound to that symbol in the appropriate [scope](https://docs.julialang.org/../variables-and-scoping/#scope-of-variables).
...
:(::)
```
The second syntactic purpose of the `:` character is to create expression objects without using the explicit [`Expr`](https://docs.julialang.org/../../base/base/#Core.Expr) constructor. This is referred to as *quoting*. The `:` character, followed by paired parentheses around a single statement of Julia code, produces an `Expr` object based on the enclosed code. Here is an example of the short form used to quote an arithmetic expression:
...
Expr
```
(to view the structure of this expression, try `ex.head` and `ex.args`, or use [`dump`](https://docs.julialang.org/../../base/io-network/#Base.dump) as above or [`Meta.@dump`](https://docs.julialang.org/../../base/io-network/#Base.Meta.@dump))

Note that equivalent expressions may be constructed using [`Meta.parse`](https://docs.julialang.org/../../base/base/#Base.Meta.parse-Tuple%7BAbstractString,%20Int64%7D) or the direct `Expr` form:
...
Expr
```
Direct construction of [`Expr`](https://docs.julialang.org/../../base/base/#Core.Expr) objects with value arguments is powerful, but `Expr` constructors can be tedious compared to "normal" Julia syntax. As an alternative, Julia allows *interpolation* of literals or expressions into quoted expressions. Interpolation is indicated by a prefix `$`.
...
The usual representation of a `quote` form in an AST is an [`Expr`](https://docs.julialang.org/../../base/base/#Core.Expr) with head `:quote`:
...
Given an expression object, one can cause Julia to evaluate (execute) it at global scope using [`eval`](https://docs.julialang.org/../../base/base/#Base.MainInclude.eval):
...
3
```
Every [module](https://docs.julialang.org/../modules/#modules) has its own [`eval`](https://docs.julialang.org/../../base/base/#Base.MainInclude.eval) function that evaluates expressions in its global scope. Expressions passed to [`eval`](https://docs.julialang.org/../../base/base/#Base.MainInclude.eval) are not limited to returning values – they can also have side-effects that alter the state of the enclosing module's environment:
...
Since expressions are just `Expr` objects which can be constructed programmatically and then evaluated, it is possible to dynamically generate arbitrary code which can then be run using [`eval`](https://docs.julialang.org/../../base/base/#Base.MainInclude.eval). Here is a simple example:
...
As hinted above, one extremely useful feature of Julia is the capability to generate and manipulate Julia code within Julia itself. We have already seen one example of a function returning [`Expr`](https://docs.julialang.org/../../base/base/#Core.Expr) objects: the [`parse`](https://docs.julialang.org/../../base/numbers/#Base.parse) function, which takes a string of Julia code and returns the corresponding `Expr`. A function can also take one or more `Expr` objects as arguments, and return another `Expr`. Here is a simple, motivating example:
...
Hello, human
```
We can view the quoted return expression using the function [`macroexpand`](https://docs.julialang.org/../../base/base/#Base.macroexpand) (**important note:** this is an extremely useful tool for debugging macros):
...
There also exists a macro [`@macroexpand`](https://docs.julialang.org/../../base/base/#Base.@macroexpand) that is perhaps a bit more convenient than the `macroexpand` function:
...
:(println("Hello, ", "human"))
```
We have already seen a function `f(::Expr...) -> Expr` in a previous section. In fact, [`macroexpand`](https://docs.julialang.org/../../base/base/#Base.macroexpand) is also such a function. So, why do macros exist?
...
```
The first call to [`println`](https://docs.julialang.org/../../base/io-network/#Base.println) is executed when [`macroexpand`](https://docs.julialang.org/../../base/base/#Base.macroexpand) is called. The resulting expression contains *only* the second `println`:
...
@name([a b]) * v
```
It is important to emphasize that macros receive their arguments as expressions, literals, or symbols. One way to explore macro arguments is to call the [`show`](https://docs.julialang.org/../../base/io-network/#Base.show-Tuple%7BIO,%20Any%7D) function within the macro body:
...
Here is a simplified definition of Julia's [`@assert`](https://docs.julialang.org/../../base/base/#Base.@assert) macro:
...
```
The [`@eval`](https://docs.julialang.org/../../base/base/#Base.@eval) macro rewrites this call to be precisely equivalent to the above longer versions. For longer blocks of generated code, the expression argument given to [`@eval`](https://docs.julialang.org/../../base/base/#Base.@eval) can be a block:
...
```
Recall from [Strings](https://docs.julialang.org/../strings/#non-standard-string-literals) that string literals prefixed by an identifier are called non-standard string literals, and can have different semantics than un-prefixed string literals. For example:
...
16
```
Note that there is no printout of [`Int64`](https://docs.julialang.org/../../base/numbers/#Core.Int64). We can see that the body of the generated function was only executed once here, for the specific set of argument types, and the result was cached. After that, for this example, the expression returned from the generated function on the first invocation was re-used as the method body. However, the actual caching behavior is an implementation-defined performance optimization, so it is invalid to depend too closely on this behavior.

The number of times a generated function is generated *might* be only once, but it *might* also be more often, or appear to not happen at all. As a consequence, you should *never* write a generated function with side effects - when, and how often, the side effects occur is undefined. (This is true for macros too - and just like for macros, the use of [`eval`](https://docs.julialang.org/../../base/base/#Base.MainInclude.eval) in a generated function is a sign that you're doing something the wrong way.) However, unlike macros, the runtime system cannot correctly handle a call to [`eval`](https://docs.julialang.org/../../base/base/#Base.MainInclude.eval), so it is disallowed.
