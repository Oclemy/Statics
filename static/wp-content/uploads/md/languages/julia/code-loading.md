...
Code inclusion is quite straightforward and simple: it evaluates the given source file in the context of the caller. Package loading is built on top of code inclusion and serves a [different purpose](https://docs.julialang.org/../modules/#modules). The rest of this chapter focuses on the behavior and mechanics of package loading.
...
These questions are answered by searching through the project environments listed in [`LOAD_PATH`](https://docs.julialang.org/../../base/constants/#Base.LOAD_PATH) for project files (`Project.toml` or `JuliaProject.toml`), manifest files (`Manifest.toml` or `JuliaManifest.toml`), or folders of source files.
