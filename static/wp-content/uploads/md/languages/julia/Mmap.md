...
`dims` is a tuple or single [`Integer`](https://docs.julialang.org/../../base/numbers/#Core.Integer) specifying the size or length of the array.

The file is passed via the stream argument, either as an open [`IOStream`](https://docs.julialang.org/../../base/io-network/#Base.IOStream) or filename string. When you initialize the stream, use `"r"` for a "read-only" array, and `"w+"` to create a new array used to write values to disk.
